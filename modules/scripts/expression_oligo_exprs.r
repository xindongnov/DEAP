...
data.affy <- read.celfiles(filenames = unlist(strsplit(file_name,',')))
eset = rma(data.affy)
...
if(!is.na(gene_path)){
  symbol=gpl_inf$gene_assignment[match(rownames(data.expr),as.character(gpl_inf$ID))]
...
if(!is.na(transcript_path)){
  transcript=gpl_inf$mrna_assignment[match(rownames(data.expr),gpl_inf$ID)]
