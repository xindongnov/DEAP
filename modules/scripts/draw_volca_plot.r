...
file <- read.table(input_path, sep = "\t", header = TRUE, stringsAsFactors = FALSE, quote = "", fill = TRUE)
...
volca <- ggplot(file,aes(logFC,-log10(adj.P.Val))) + 
  ...
  theme_bw()+
  geom_hline(aes(yintercept=-log10(fdr_setting)),linetype="dashed",color="grey",size=0.5)+
  geom_vline(aes(xintercept=logFC_setting),linetype="dashed",color="grey",size=0.5)+
  geom_vline(aes(xintercept=-logFC_setting),linetype="dashed",color="grey",size=0.5)+theme(axis.text.x = element_text(size = 10))+
