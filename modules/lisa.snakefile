...
rule lisa_run:
    input:
        "analysis/{run}/lisa/{treatment}_{geneType}/{treatment}.{geneType}.txt"
    output:
        "analysis/{run}/lisa/{treatment}_{geneType}/{treatment}.{geneType}.txt.1000.lisa_direct.csv",
        "analysis/{run}/lisa/{treatment}_{geneType}/{treatment}.{geneType}.txt.profile",
        "analysis/{run}/lisa/{treatment}_{geneType}/{treatment}.{geneType}.txt.Snakefile.model",
        "analysis/{run}/lisa/{treatment}_{geneType}/{treatment}.{geneType}.txt.yml"
    params:
        prefix=lambda wildcards: "%s.%s.txt" % (wildcards.treatment,wildcards.geneType),
        species="hg38",
        lisa_path=config["lisa_path"],
    threads: 8
    run:
        cmd = "cd analysis/%s/lisa/%s_%s/; " % (wildcards.run,wildcards.treatment,wildcards.geneType)
