...
def lisa_targets(wildcards):
    ...
    #                 n = subprocess.check_output("wc -l %s" % checkpoints.lisa_copy_file.get(run=run,condition="%s_%s" % (treat,ctrl),geneType=geneType).output[0],shell=True)
    #                 n = int(n.decode("utf-8").strip().split()[0])
    #                 if n != 0:
    #                     ls.append("analysis/%s/lisa/%s_%s_%s/%s_%s.%s.txt.1000.lisa_direct.csv" % (run,treat,ctrl,geneType,treat,ctrl,geneType))
                ...
                # n = subprocess.check_output("wc -l %s" % checkpoints.lisa_copy_file.get(run=run,condition="%s_%s" % (treat,ctrl),geneType="downRegGenes").output[0],shell=True).decode("utf-8").strip().split()[0]
                # n = int(n)
                # if n != 0:
...
#     input:
#         "analysis/{run}/expression/{run}_results/{condition}.{geneType}.txt"
#     output:
#         "analysis/{run}/lisa/{condition}_{geneType}/{condition}.{geneType}.txt"
#     shell:
#         "cp {input} {output}"
