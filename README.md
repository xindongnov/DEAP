...
This is a table that define samples' relationship and the way you would like to do differential expression genes among samples. Except `compare_XXX`, other headers should not be modified.  
  ...
  - **The 6th column (compare_XXX)**: The samples that you want to perform a Differential Expression to using limma and DEseq. The "control" should be marked with a `0`, and the "treatment" should be marked with a `1`. You are allowed to have as many `compare_XXXX` columns as you want at the end.  
